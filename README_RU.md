...
[![Обзор](docs/screenshots/02-dashboard.png)](docs/SCREENSHOTS.md)

Дашборд с живыми метриками, слева — разделы панели. Ещё 16 экранов с подробным
разбором каждого: **[docs/SCREENSHOTS.md](docs/SCREENSHOTS.md)**.

| | |
|---|---|
| [![Сервер](docs/screenshots/04-server-show.png)](docs/SCREENSHOTS.md#4-обзор-сервера) | [![Журнал деплоя](docs/screenshots/10-deployment-log.png)](docs/SCREENSHOTS.md#10-журнал-деплоя) |
| Обзор сервера: метрики, спарклайны, версии тулчейна | Деплой: шаги пайплайна и живой журнал |
| [![Установка ПО](docs/screenshots/05-provision.png)](docs/SCREENSHOTS.md#5-установка-по-провижининг) | [![Проекты](docs/screenshots/06-projects.png)](docs/SCREENSHOTS.md#6-проекты) |
| Провижининг чистого Ubuntu в один клик | BEAM-приложения со всех серверов |
...
Проверить, что всё на месте, можно в любой момент:

```bash
sudo bash scripts/doctor.sh
```

Проверяются конфигурация, релиз и симлинк, служба и её автозапуск, ответ по HTTP,
nginx, срок сертификата, база и наличие администратора, беспарольный sudo,
резервные копии и SSH-ключ панели. Установщик и обновление запускают эту проверку
автоматически в самом конце.

**Что работает само, без вашего участия:**

* служба перезапускается при падении (`Restart=on-failure`);
* миграции накатываются при каждом старте (`ExecStartPre`);
* база выгружается каждую ночь в `/var/backups`, хранится 14 дней
  (`beam-panel-backup.timer`);
* сертификат Let's Encrypt продлевается таймером certbot;
* метрики старше 14 дней удаляются, зависшие деплои закрываются, статусы
  проектов обновляются — раз в 15 минут внутри самой панели;
* графики не пустеют после перезапуска: панель восстанавливает историю метрик
  из PostgreSQL при старте.
